...
function jobsReducer(state = {ready: {}, inProgress: {}, done: {}}, action) {
  var job = action.data
...
function performJob(job) {
  switch (job.type) {
    case 'request':
      return httpWorker(job)
    default:
      return console.warn(`Unknown job type: ${job.type}`)
  }
}

function httpWorker(job) {
  store.dispatch('job.start', job.data.name)
  axios.get({
      url: job.url,
      type: job.method
    })
...
//     name: 'syncArticles',
//     type: 'get',
//     url: state.entities.articles.url
//   }
// })
...
function onStoreChanged() {
  ...
  state.jobs.ready.map(job => performJob(job))
}
